...
START_URL = '/test' # no protocol or host
SLEEP_SECONDS = 5
WIFI_DEV_PREFIX = 'wl'
ESC='\033'
...
def main():
    ...
    for dev in usb_wifi_ifaces: # now start using the connections
        ...
        if usb_wifi_ifaces[dev].get('status', None) != 'UP':
            continue
...
def monitor_dev(dev):
    ...
    while run_flag:
        ...
        #print(client_ip)
        if client_ip:
            header, stat_json = get_html(client_ip, url, port=SERVER_PORT)
...
def one_dev(dev):
    ...
    while run_flag:
        ...
        if client_ip:
            ...
            connect_wifi(dev)
        time.sleep(random.uniform(.5 * SLEEP_SECONDS, 1.5 * SLEEP_SECONDS))

def get_html(client_ip, page_url, port=SERVER_PORT, server_ip=SERVER_IP):
